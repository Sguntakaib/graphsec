...
- Aligns each answer option to either:
  ...
  • partial mitigation (status=partial)

This complements the heuristic rules in StrideRuleEngine by using explicit mappings
from the questionnaires. Mappings here cover common WebApp, API, and Database
questions and can be extended incrementally.
"""
...
class OptionImpact:
    ...
    residual_risk: float
    status_override: Optional[ThreatStatus] = None  # Allow explicit PARTIAL marking

# Helper to resolve final status
...
WEBAPP_MAPPINGS: Dict[str, Dict[str, List[OptionImpact]]] = {
    ...
    "webapp_authentication_method": {
        "No Authentication": [
            OptionImpact(
                StrideCategory.SPOOFING, "increase",
                "No Authentication Allows Spoofing",
                "Absence of authentication enables identity spoofing and unauthorized access.",
                ["Introduce OAuth2/OIDC or SAML", "Add MFA across user flows"], 8.0,
            )
        ],
        "Username/Password only": [
            OptionImpact(
                StrideCategory.SPOOFING, "increase",
                "Weak Authentication Susceptible to Credential Abuse",
                "Basic username/password without MFA is vulnerable to brute force and credential stuffing.",
                ["Enable MFA", "Rate limit logins", "Block breached passwords"], 6.5,
            )
        ],
        "Username/Password with MFA": [
            OptionImpact(
                StrideCategory.SPOOFING, "mitigate",
                "Multi-Factor Authentication Mitigates Spoofing",
                "MFA significantly raises the bar against identity spoofing.",
                ["Maintain MFA for privileged/external access"], 2.0,
            )
        ],
        "OAuth2/OIDC": [
            OptionImpact(
                StrideCategory.SPOOFING, "mitigate",
                "Federated Auth Mitigates Spoofing",
                "Standards-based token auth reduces spoofing risk when configured securely.",
                ["Short-lived tokens", "Harden token validation"], 1.5,
            )
        ],
        "SAML": [
            OptionImpact(
                StrideCategory.SPOOFING, "mitigate",
                "SAML SSO Mitigates Spoofing",
                "Enterprise SSO with strong policies reduces identity spoofing.",
                ["Strong IdP policies", "Session binding"], 1.8,
            )
        ],
    },
    # Input validation
    "webapp_input_validation": {
        "No validation": [
            OptionImpact(
                StrideCategory.TAMPERING, "increase",
                "Missing Input Validation Enables Tampering",
                "Lack of server-side validation allows injection and data corruption.",
                ["Implement comprehensive server-side validation", "Sanitization"], 8.0,
            )
        ],
        "Client-side only": [
            OptionImpact(
                ...
                "Attackers can bypass client logic leading to tampering and injections.",
                ["Enforce server-side validation"], 7.0,
            )
        ],
        "Basic validation": [
            OptionImpact(
                StrideCategory.TAMPERING, "partial",
                "Basic Validation Provides Limited Protection",
                "Simple checks reduce but do not eliminate injection/tampering risk.",
                ["Adopt centralized validation with schemas"], 4.5,
                status_override=ThreatStatus.PARTIAL,
            )
        ],
        "Comprehensive server-side validation": [
            OptionImpact(
                StrideCategory.TAMPERING, "mitigate",
                "Strong Validation Mitigates Tampering",
                "Robust server-side validation reduces injection/tampering risk.",
                ["Maintain validation libraries", "Centralize validation"], 2.0,
            )
        ],
    },
    # HTTPS
    "webapp_https_enforcement": {
        "HTTP only": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "increase",
                "Unencrypted Traffic Exposes Data",
                "HTTP allows interception and leakage of sensitive data.",
                ["Force HTTPS", "Enable HSTS"], 7.0,
            )
        ],
        "Mixed HTTP/HTTPS": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "increase",
                "Mixed Transport Security Risks",
                "Inconsistent HTTPS leads to leakage on unencrypted paths.",
                ["Enforce HTTPS everywhere", "Eliminate HTTP endpoints"], 6.0,
            )
        ],
        "HTTPS preferred": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "partial",
                "HTTPS Preferred but Not Enforced",
                "Some risk remains where HTTP may still be accepted.",
                ["Redirect HTTP->HTTPS", "Harden TLS"], 3.0,
                status_override=ThreatStatus.PARTIAL,
            )
        ],
        "HTTPS only (HSTS enabled)": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "mitigate",
                "Strict HTTPS Mitigates Info Disclosure",
                "HSTS prevents downgrade and ensures encrypted transit.",
                ["Monitor TLS posture"], 1.5,
            )
        ],
    },
    # Sessions
    "webapp_session_management": {
        "No session management": [
            OptionImpact(
                StrideCategory.ELEVATION_OF_PRIVILEGE, "increase",
                "No Sessions Facilitate Privilege Abuse",
                "Lack of session controls enables fixation/hijacking patterns.",
                ["Use secure cookies", "Rotate session IDs", "Short timeouts"], 6.5,
            )
        ],
        "Basic sessions": [
            OptionImpact(
                StrideCategory.ELEVATION_OF_PRIVILEGE, "partial",
                "Basic Sessions Provide Limited Protection",
                "Minimal security features leave room for escalation attacks.",
                ["Harden cookie flags", "Implement rotation"], 4.0,
                status_override=ThreatStatus.PARTIAL,
            )
        ],
        "Secure session management": [
            OptionImpact(
                StrideCategory.ELEVATION_OF_PRIVILEGE, "mitigate",
                "Secure Sessions Mitigate Elevation",
                "Strong session practices reduce privilege escalation avenues.",
                ["Harden SameSite/HttpOnly/Secure flags"], 2.0,
            )
        ],
    },
    # Error handling
    "webapp_error_handling": {
        "No error handling": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "increase",
                "No Error Handling Causes Info Disclosure",
                "Unhandled exceptions and verbose responses leak sensitive information.",
                ["Centralize error handlers", "Return generic messages"], 6.5,
            )
        ],
        "Detailed error messages": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "increase",
                "Detailed Errors Reveal Sensitive Information",
                "Technical details aid attackers in reconnaissance.",
                ["Use generic responses", "Secure server-side logging"], 5.5,
            )
        ],
        "Generic error messages": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "partial",
                "Generic Errors Reduce But Don’t Eliminate Risk",
                "Safer user messages with possible backend leakage if not configured.",
                ["Review error propagation"], 3.0,
                status_override=ThreatStatus.PARTIAL,
            )
        ],
        "Secure error handling (no info disclosure)": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "mitigate",
                "Secure Error Handling Mitigates Disclosure",
                "Sensitive details are never exposed in client responses.",
                ["Centralize handlers", "Security-focused logging"], 1.5,
            )
        ],
    },
    ...
    "webapp_logging_monitoring": {
        "No logging": [
            OptionImpact(
                StrideCategory.REPUDIATION, "increase",
                "No Logging Enables Repudiation",
                "Lack of audit trails prevents accountability and forensics.",
                ["Enable security event logging", "Centralize logs"], 6.0,
            )
        ],
        "Minimal logging": [
            OptionImpact(
                StrideCategory.REPUDIATION, "increase",
                "Minimal Logging Insufficient for Non-Repudiation",
                "Limited events reduce traceability and investigative value.",
                ["Expand audit scope", "Alerts on anomalies"], 5.0,
            )
        ],
        "Basic application logging": [
            OptionImpact(
                StrideCategory.REPUDIATION, "partial",
                "Basic Logging Provides Partial Evidence",
                "Covers common events but may miss security-sensitive trails.",
                ["Add auth/access logs", "Retention & monitoring"], 3.0,
                status_override=ThreatStatus.PARTIAL,
            )
        ],
        "Comprehensive security logging": [
            OptionImpact(
                StrideCategory.REPUDIATION, "mitigate",
                "Comprehensive Logging Mitigates Repudiation",
                "Full auditability supports investigations and compliance.",
                ["SIEM integration", "Alerting"], 1.5,
            )
        ],
    },
    # Security headers (Info Disclosure + Tampering often)
    "webapp_security_headers": {
        "No security headers": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "increase",
                "Missing Security Headers Increase Risk",
                "Lack of CSP/HSTS/XFO leaves room for disclosure and clickjacking.",
                ["Implement CSP, HSTS, XFO, XCTO"], 6.0,
            )
        ],
        "Minimal headers": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "increase",
                "Minimal Headers Provide Insufficient Protection",
                "Common web-based attacks remain feasible.",
                ["Adopt comprehensive header set"], 4.5,
            )
        ],
        "Basic headers": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "partial",
                "Basic Headers Partially Reduce Risk",
                "Some protections present but not comprehensive.",
                ["Harden CSP & HSTS"], 3.0,
                status_override=ThreatStatus.PARTIAL,
            )
        ],
        "Comprehensive security headers": [
            OptionImpact(
                StrideCategory.INFORMATION_DISCLOSURE, "mitigate",
                "Comprehensive Headers Mitigate Disclosure",
                "Well-configured CSP/HSTS/XFO/XCTO protect against several web risks.",
                ["Continuous header audits"], 1.5,
            )
        ],
    },
}
...
API_MAPPINGS: Dict[str, Dict[str, List[OptionImpact]]] = {
    "api_authentication_method": {
        ...
        "API Keys": [
            OptionImpact(StrideCategory.SPOOFING, "partial", "API Keys Provide Moderate Protection", "Key-based auth depends on secure storage/rotation.", ["Rotate keys", "Restrict scope"], 3.5, status_override=ThreatStatus.PARTIAL)
        ],
        "OAuth2/JWT": [
    ...
    "api_authorization_model": {
        ...
        "API key permissions": [
            OptionImpact(StrideCategory.ELEVATION_OF_PRIVILEGE, "partial", "API Key Permissions Offer Limited Control", "Key-based permissions are coarse-grained.", ["Prefer RBAC with scopes"], 4.0, status_override=ThreatStatus.PARTIAL)
        ],
        "Simple role-based": [
            OptionImpact(StrideCategory.ELEVATION_OF_PRIVILEGE, "partial", "Basic RBAC Partially Mitigates Elevation", "Broad roles may still grant excessive privileges.", ["Refine scope & roles"], 3.5, status_override=ThreatStatus.PARTIAL)
        ],
    ...
    "api_rate_limiting": {
        ...
        "Basic throttling": [
            OptionImpact(StrideCategory.DENIAL_OF_SERVICE, "partial", "Basic Throttling Partially Prevents DoS", "Simple caps reduce but don't stop abuse.", ["Introduce dynamic limits"], 4.0, status_override=ThreatStatus.PARTIAL)
        ],
...
DATABASE_MAPPINGS: Dict[str, Dict[str, List[OptionImpact]]] = {
    "database_authentication": {
        ...
        "Basic authentication": [
            OptionImpact(StrideCategory.SPOOFING, "increase", "Weak DB Auth Susceptible to Abuse", "Simple credentials are easily brute-forced or leaked.", ["Strong auth + MFA"], 6.5)
        ],

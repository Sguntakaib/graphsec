...
function AppContent() {
  ...
  const handleDependentNodeCreation = async (dependentNodeTypes, parentAnswers) => {
    ...
    // Create nodes for each dependent type
    ...
    // Create edges connecting parent to dependent nodes
    ...
    // Queue questionnaires for the new nodes
    setQuestionnaireQueue(newNodes);
    setCurrentQueueIndex(0);
    
    // Start with the first dependent node questionnaire
    if (newNodes.length > 0) {
      setCurrentQuestionnaireNode(newNodes[0]);
    }
  };

...
BASE_URL = "https://sec-context-modal.preview.emergentagent.com/api"
...
class SecurityModelingAPITester:
    ...
    # ============================================================================
    # EXPANDED INTELLIGENT NODES ENDPOINTS - PHASE 1 ENHANCED APIs TESTING
    # ============================================================================
    ...
    def run_phase1_core_loop_tests_only(self):
        """Run only the Phase 1 Core Loop tests for focused testing"""
        print("🚀 PHASE 1 CORE LOOP COMPLETION TESTING - COMPREHENSIVE BACKEND VALIDATION")
        print(f"📡 Backend URL: {self.base_url}")
        print("=" * 80)
        
        # PHASE 1 CORE LOOP COMPLETION TESTS - CRITICAL PRIORITY
        print("🎯 PRIORITY 1 - CORE QUESTIONNAIRE COMPLETION FLOW")
        print("🎯 PRIORITY 2 - FINDINGS MANAGEMENT SYSTEM") 
        print("🎯 PRIORITY 3 - ENHANCED SIMULATION & RULE EVALUATION")
        print("🎯 PRIORITY 4 - MERGED QUESTIONNAIRE PROMPTS")
        print("🎯 PRIORITY 5 - PERFORMANCE & ERROR HANDLING")
        print("=" * 80)
        
        core_loop_tests = [
            self.test_questionnaire_completion_flow,
            self.test_findings_management_crud,
            self.test_enhanced_simulation_endpoint,
            self.test_enhanced_rule_evaluation,
            self.test_merged_questionnaire_prompts,
            self.test_performance_and_error_handling
        ]
        
        # Run core loop tests first (critical priority)
        core_passed = 0
        core_failed = 0
        
        print("🔥 RUNNING PHASE 1 CORE LOOP TESTS...")
        print("-" * 80)
        
        for test in core_loop_tests:
            try:
                if test():
                    core_passed += 1
                else:
        ...
        print("=" * 80)
        print("🎯 PHASE 1 CORE LOOP COMPLETION RESULTS")
        print(f"✅ Core Loop Passed: {core_passed}")
        print(f"❌ Core Loop Failed: {core_failed}")
        print(f"📊 Core Loop Success Rate: {(core_passed/(core_passed+core_failed)*100):.1f}%")
        print("=" * 80)
        
        # Show critical failures
        if core_failed > 0:
            print("\n🚨 CRITICAL CORE LOOP FAILURES:")
            for result in self.test_results:
                if not result["success"]:
                    print(f"   ❌ {result['test']}: {result['message']}")
        
        # Determine overall success
        if core_passed == len(core_loop_tests):
            print("\n🎉 PHASE 1 CORE LOOP COMPLETION: ALL CRITICAL TESTS PASSED!")
            print("✅ Revolutionary questionnaire completion flow validated")
            print("✅ Automated findings generation confirmed") 
            print("✅ Framework compliance mapping verified")
            print("✅ Performance requirements met")
            return True
        else:
            print(f"\n⚠️ PHASE 1 CORE LOOP: {core_failed} CRITICAL TESTS FAILED")
            print("🔧 Main agent needs to address core loop issues before production")
            return False

    # ============================================================================
    # PHASE 1 CORE LOOP CRITICAL ENDPOINTS TESTS
    # ============================================================================
    ...
    # Phase 1 Enhanced APIs - Specific Review Request Tests
    def test_multi_level_questionnaires(self):
        """Test GET /api/expanded-nodes/EC2/questionnaire/{level} for basic/advanced/expert"""
        levels = [
            ("basic", 5, 8),      # Should return 5-8 questions
            ("advanced", 15, 20), # Should return 15-20 questions  
            ("expert", 25, 30)    # Should return 25-30 questions
        ]
        
        for level, min_questions, max_questions in levels:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "questions" not in data:
                        self.log_test(f"Multi-Level Questionnaire - EC2/{level}", False, 
                    ...
                    if min_questions <= question_count <= max_questions:
                        self.log_test(f"Multi-Level Questionnaire - EC2/{level}", True, 
                                    f"Question count {question_count} within expected range ({min_questions}-{max_questions})")
                    else:
                        self.log_test(f"Multi-Level Questionnaire - EC2/{level}", False, 
                                    f"Question count {question_count} outside expected range ({min_questions}-{max_questions})")
                        return False
                ...
                self.log_test(f"Multi-Level Questionnaire - EC2/{level}", False, f"Error: {str(e)}")
                return False
        
        return True

    def test_categories_api(self):
        """Test GET /api/expanded-nodes/categories - verify direct category mapping"""
        try:
            response = self.session.get(f"{self.base_url}/expanded-nodes/categories")
            ...
            if response.status_code == 200:
                ...
                # Verify it returns direct category mapping instead of categories array
                if isinstance(data, dict) and "categories" not in data:
                    # Should be direct mapping like {"Infrastructure": [...], "Security": [...]}
                    category_keys = list(data.keys())
                    if category_keys:
                        self.log_test("Categories API", True, 
                                    f"Returns direct category mapping with {len(category_keys)} categories: {category_keys}")
                        return True
                    else:
                        self.log_test("Categories API", False, "Empty category mapping returned")
                        return False
                elif isinstance(data, dict) and "categories" in data:
                    self.log_test("Categories API", False, 
                                "Returns categories array instead of direct mapping")
                    return False
                else:
                    self.log_test("Categories API", False, 
    ...
    def test_enhanced_risk_calculation(self):
        """Test POST /api/expanded-nodes/EC2/calculate-risk - verify 'risk_factors' field"""
        try:
            # Test data for EC2 risk calculation
            test_data = {
                "responses": {
                    "instance_type": "t3.large",
                    "security_groups": "restrictive",
            ...
            if response.status_code == 200:
                ...
                # Verify 'risk_factors' field is present
                if "risk_factors" in data:
                    risk_factors = data.get("risk_factors", {})
                    self.log_test("Enhanced Risk Calculation", True, 
                                f"'risk_factors' field present with {len(risk_factors)} factors")
                    return True
                else:
                    self.log_test("Enhanced Risk Calculation", False, 
                                "'risk_factors' field missing from response")
                    return False
            else:
                self.log_test("Enhanced Risk Calculation", False, 
                            f"HTTP {response.status_code}: {response.text}")
    ...
    def test_bulk_risk_assessment(self):
        """Test POST /api/expanded-nodes/bulk-risk-assessment - verify field names"""
        try:
            # Test data for bulk assessment
            test_data = {
                "assessments": [
                    {
                        "node_type": "EC2",
            ...
            if response.status_code == 200:
                ...
                if not missing_fields:
                    ...
                    self.log_test("Bulk Risk Assessment", True, 
                                f"Correct field names: {individual_count} individual assessments, overall summary present")
                    return True
                else:
                    self.log_test("Bulk Risk Assessment", False, 
    ...
    def test_threat_intelligence_summary(self):
        """Test POST /api/expanded-nodes/threat-intelligence-summary - verify field names"""
        try:
            # Test data for threat intelligence summary
            test_data = {
            ...
            if response.status_code == 200:
                ...
                if not missing_fields:
                    ...
                    self.log_test("Threat Intelligence Summary", True, 
                                f"Correct field names: {node_summaries_count} node summaries, {cross_threats_count} cross-cutting threats")
                    return True
                else:
                    self.log_test("Threat Intelligence Summary", False, 
                                f"Missing expected fields: {missing_fields}")
    ...
    def test_threat_intelligence_summary_review(self):
        """Test POST /api/expanded-nodes/threat-intelligence-summary - verify field names"""
        try:
            # Test data for threat intelligence summary
            test_data = {
            ...
            if response.status_code == 200:
                ...
                if not missing_fields:
                    node_summaries_count = len(data.get("node_type_summaries", {}))
                    cross_threats_count = len(data.get("cross_cutting_threats", []))
                    self.log_test("Threat Intelligence Summary Review", True, 
                                f"Correct field names: {node_summaries_count} node summaries, {cross_threats_count} cross-cutting threats")
                    return True
                else:
                    self.log_test("Threat Intelligence Summary Review", False, 
                                f"Missing expected fields: {missing_fields}")
                    return False
            else:
                self.log_test("Threat Intelligence Summary Review", False, 
            ...
            self.log_test("Threat Intelligence Summary Review", False, f"Error: {str(e)}")
            return False
    ...
    def test_phase2_questionnaire_loader_system(self):
        """Test the questionnaire loader system with new files"""
        try:
            # Test that the questionnaire loader is working with file-based system
            node_types = ["WebApp", "API", "Database"]
            levels = ["basic", "advanced", "expert"]
            
            for node_type in node_types:
                for level in levels:
                    response = self.session.get(f"{self.base_url}/questionnaires/{node_type}?level={level}")
                    
                    if response.status_code != 200:
                        self.log_test("Phase 2 Questionnaire Loader System", False, 
                                    f"Loader failed for {node_type} {level}: HTTP {response.status_code}")
                        return False
                    
                    data = response.json()
                    
                    # Verify loader system metadata
                    if "loader_version" in data.get("metadata", {}):
                        loader_version = data["metadata"]["loader_version"]
                        if not loader_version:
                            self.log_test("Phase 2 Questionnaire Loader System", False, 
                                        f"Missing loader version for {node_type} {level}")
                            return False
            
            self.log_test("Phase 2 Questionnaire Loader System", True, 
                        "✅ Questionnaire loader system working with file-based questionnaires")
            return True
            
        except Exception as e:
            self.log_test("Phase 2 Questionnaire Loader System", False, f"Error: {str(e)}")
            return False
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if missing_fields:
                        self.log_test(f"Phase 2 Questionnaire Primary - {node_subtype} {level}", False, 
                    ...
                    self.log_test(f"Phase 2 Questionnaire Primary - {node_subtype} {level}", True, 
                ...
                self.log_test(f"Phase 2 Questionnaire Primary - {node_subtype} {level}", False, f"Error: {str(e)}")
                all_passed = False
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if missing_fields:
                        self.log_test(f"Phase 2 Questionnaire Alternative - {node_subtype} {level}", False, 
                                    f"Missing required fields: {missing_fields}")
                        all_passed = False
                        continue
                    ...
                    self.log_test(f"Phase 2 Questionnaire Alternative - {node_subtype} {level}", True, 
                ...
                self.log_test(f"Phase 2 Questionnaire Alternative - {node_subtype} {level}", False, f"Error: {str(e)}")
                all_passed = False
        
        return all_passed

    def test_phase2_questionnaire_response_format_comparison(self):
        """Compare response formats between primary and alternative endpoints"""
        test_case = {"node_subtype": "WebApp", "level": "basic"}
        ...
        try:
            ...
            if primary_response.status_code == 200 and alternative_response.status_code == 200:
                ...
                if differences:
                    self.log_test("Phase 2 Questionnaire Format Comparison", True, 
                ...
                self.log_test("Phase 2 Questionnaire Format Comparison", False, 
    ...
    # =============================================================================
    # PHASE 1 VULNERABILITY SYSTEM TESTS
    # =============================================================================
    
    def create_realistic_webapp_questionnaire_data(self):
        """Create realistic WebApp questionnaire data for vulnerability testing"""
        return {
            "webapp_authentication_method": "Username/Password only",
            "webapp_input_validation": "Client-side only", 
            "webapp_https_enforcement": "Mixed HTTP/HTTPS",
            "webapp_session_management": "Basic sessions",
            "webapp_data_encryption": "No encryption",
            "webapp_security_headers": "No security headers",
            "webapp_csrf_protection": "No CSRF protection",
            "webapp_xss_protection": "No XSS protection",
            "webapp_logging_monitoring": "No logging",
            "webapp_rate_limiting": "No rate limiting",
            "webapp_error_handling": "Detailed error messages"
        }
    
    def create_realistic_api_questionnaire_data(self):
        """Create realistic API questionnaire data for vulnerability testing"""
        return {
    ...
    def create_realistic_database_questionnaire_data(self):
        """Create realistic Database questionnaire data for vulnerability testing"""
        return {
            "database_authentication": "No authentication",
            ...
            "database_backup_strategy": "No backup strategy",
            "database_patch_management": "No patch management",
            "database_network_security": "No network restrictions",
            "database_logging": "No logging"
        }
    
    def test_vulnerability_analyze_webapp_node(self):
        """Test POST /api/vulnerabilities/analyze/{node_id} for WebApp node"""
        try:
            node_id = str(uuid.uuid4())
            questionnaire_data = self.create_realistic_webapp_questionnaire_data()
            ...
            if response.status_code == 200:
                ...
                missing_fields = [f for f in required_fields if f not in data]
                if missing_fields:
                    self.log_test("Vulnerability Analysis - WebApp", False, f"Missing fields: {missing_fields}")
                    return False
                
                # Verify vulnerability generation
                total_vulns = data.get("total_vulnerabilities", 0)
                vulnerability_nodes = data.get("vulnerability_nodes", [])
                
                if total_vulns == 0:
                    self.log_test("Vulnerability Analysis - WebApp", False, "No vulnerabilities generated for insecure WebApp")
                    return False
                
                # Check for OWASP Top 10 2023 categories
                owasp_categories = [vuln.get("owasp_category", "") for vuln in vulnerability_nodes]
                expected_categories = [
                ...
                found_categories = [cat for cat in expected_categories if any(cat in owasp_cat for owasp_cat in owasp_categories)]
                
                # Verify severity distribution
                severity_counts = data.get("vulnerabilities_by_severity", {})
                critical_high_count = severity_counts.get("Critical", 0) + severity_counts.get("High", 0)
                
                if critical_high_count == 0:
                    self.log_test("Vulnerability Analysis - WebApp", False, "No Critical/High severity vulnerabilities for insecure configuration")
                    return False
                
                # Verify MITRE technique mapping
                mitre_techniques = []
                for vuln in vulnerability_nodes:
                    mitre_techniques.extend(vuln.get("mitre_techniques", []))
                
                if not mitre_techniques:
                    self.log_test("Vulnerability Analysis - WebApp", False, "No MITRE techniques mapped")
                    return False
                
                # Verify remediation steps
                remediation_provided = all(vuln.get("remediation_steps") for vuln in vulnerability_nodes)
                if not remediation_provided:
                    self.log_test("Vulnerability Analysis - WebApp", False, "Missing remediation steps")
                    return False
                
                self.log_test("Vulnerability Analysis - WebApp", True, 
                            f"Generated {total_vulns} vulnerabilities, {len(found_categories)} OWASP categories, "
                            f"{critical_high_count} Critical/High severity, {len(set(mitre_techniques))} MITRE techniques")
                return True
            else:
                self.log_test("Vulnerability Analysis - WebApp", False, f"HTTP {response.status_code}: {response.text}")
                return False
                
        except Exception as e:
            self.log_test("Vulnerability Analysis - WebApp", False, f"Error: {str(e)}")
            return False
    
    def test_vulnerability_analyze_api_node(self):
        """Test POST /api/vulnerabilities/analyze/{node_id} for API node"""
        try:
            node_id = str(uuid.uuid4())
            questionnaire_data = self.create_realistic_api_questionnaire_data()
            ...
            if response.status_code == 200:
                ...
                if total_vulns == 0:
                    self.log_test("Vulnerability Analysis - API", False, "No vulnerabilities generated for insecure API")
                    return False
                
                # Check for API-specific vulnerabilities
                api_specific_vulns = [
                    "API Unauthorized Access", "API Injection", "API Abuse", 
                    "API Data Interception", "Over-Privileged Access"
                ]
                
                found_api_vulns = []
                for vuln in vulnerability_nodes:
                    vuln_name = vuln.get("name", "")
                    for api_vuln in api_specific_vulns:
                        if api_vuln.lower() in vuln_name.lower():
                            found_api_vulns.append(api_vuln)
                            break
                
                if not found_api_vulns:
                    self.log_test("Vulnerability Analysis - API", False, "No API-specific vulnerabilities found")
                    return False
                
                # Verify orbital positioning around parent node
                positioned_vulns = [v for v in vulnerability_nodes if v.get("position")]
                if len(positioned_vulns) != len(vulnerability_nodes):
                    self.log_test("Vulnerability Analysis - API", False, "Not all vulnerabilities have position data")
                    return False
                
                self.log_test("Vulnerability Analysis - API", True, 
                            f"Generated {total_vulns} API vulnerabilities, found: {found_api_vulns}")
                return True
            else:
                self.log_test("Vulnerability Analysis - API", False, f"HTTP {response.status_code}: {response.text}")
                return False
                
        except Exception as e:
            self.log_test("Vulnerability Analysis - API", False, f"Error: {str(e)}")
            return False
    
    def test_vulnerability_analyze_database_node(self):
        """Test POST /api/vulnerabilities/analyze/{node_id} for Database node"""
        try:
            node_id = str(uuid.uuid4())
            questionnaire_data = self.create_realistic_database_questionnaire_data()
            ...
            if response.status_code == 200:
                ...
                if total_vulns == 0:
                    self.log_test("Vulnerability Analysis - Database", False, "No vulnerabilities generated for insecure Database")
                    return False
                
                # Check for Database-specific vulnerabilities
                db_specific_vulns = [
                    "Database Data Exposure", "Database Unauthorized Access", 
                    "Database Privilege Escalation", "Database Connection Interception",
                    "Data Loss Risk", "Database Tampering"
                ]
                
                found_db_vulns = []
                for vuln in vulnerability_nodes:
                    vuln_name = vuln.get("name", "")
                    for db_vuln in db_specific_vulns:
                        if db_vuln.lower() in vuln_name.lower():
                            found_db_vulns.append(db_vuln)
                            break
                
                if not found_db_vulns:
                    self.log_test("Vulnerability Analysis - Database", False, "No Database-specific vulnerabilities found")
                    return False
                
                # Verify encryption-related vulnerabilities are present
                encryption_vulns = [v for v in vulnerability_nodes if "encryption" in v.get("name", "").lower() or "data exposure" in v.get("name", "").lower()]
                if not encryption_vulns:
                    self.log_test("Vulnerability Analysis - Database", False, "No encryption-related vulnerabilities found")
                    return False
                
                self.log_test("Vulnerability Analysis - Database", True, 
                            f"Generated {total_vulns} Database vulnerabilities, found: {found_db_vulns}")
                return True
            else:
                self.log_test("Vulnerability Analysis - Database", False, f"HTTP {response.status_code}: {response.text}")
                return False
                
        except Exception as e:
            self.log_test("Vulnerability Analysis - Database", False, f"Error: {str(e)}")
            return False
    
    def test_get_node_vulnerabilities(self):
        """Test GET /api/vulnerabilities/{node_id}"""
        try:
            # First create a vulnerability analysis
            node_id = str(uuid.uuid4())
            questionnaire_data = self.create_realistic_webapp_questionnaire_data()
            ...
            if create_response.status_code != 200:
                self.log_test("Get Node Vulnerabilities", False, "Failed to create initial analysis")
                return False
            
            # Now retrieve the vulnerabilities
            response = self.session.get(f"{self.base_url}/vulnerabilities/{node_id}")
            
            if response.status_code == 200:
                data = response.json()
                
                # Verify structure matches analysis result
                required_fields = [
                    "node_id", "total_vulnerabilities", "vulnerability_nodes",
                    "overall_risk_score", "recommendations"
                ]
                
                missing_fields = [f for f in required_fields if f not in data]
                if missing_fields:
                    self.log_test("Get Node Vulnerabilities", False, f"Missing fields: {missing_fields}")
                    return False
                
                total_vulns = data.get("total_vulnerabilities", 0)
                if total_vulns == 0:
                    self.log_test("Get Node Vulnerabilities", False, "No vulnerabilities retrieved")
                    return False
                
                self.log_test("Get Node Vulnerabilities", True, 
                            f"Retrieved {total_vulns} vulnerabilities for node {node_id}")
                return True
            else:
                self.log_test("Get Node Vulnerabilities", False, f"HTTP {response.status_code}: {response.text}")
                return False
                
        except Exception as e:
            self.log_test("Get Node Vulnerabilities", False, f"Error: {str(e)}")
            return False
    
    def test_vulnerability_remediation_guidance(self):
        """Test POST /api/vulnerabilities/remediate/{vulnerability_id}"""
        try:
            # First create a vulnerability analysis to get vulnerability IDs
            node_id = str(uuid.uuid4())
            questionnaire_data = self.create_realistic_webapp_questionnaire_data()
            ...
            if create_response.status_code != 200:
                self.log_test("Vulnerability Remediation", False, "Failed to create initial analysis")
                return False
            
            analysis_data = create_response.json()
            vulnerability_nodes = analysis_data.get("vulnerability_nodes", [])
            
            if not vulnerability_nodes:
                self.log_test("Vulnerability Remediation", False, "No vulnerabilities to test remediation")
                return False
            
            # Test remediation for first vulnerability
            vulnerability_id = vulnerability_nodes[0]["id"]
            
            response = self.session.post(f"{self.base_url}/vulnerabilities/remediate/{vulnerability_id}")
            
            if response.status_code == 200:
                data = response.json()
                
                # Check remediation guidance structure
                required_fields = [
                    "vulnerability_id", "vulnerability_name", "severity", "category",
                    "immediate_steps", "implementation_priority", "estimated_effort"
                ]
                
                missing_fields = [f for f in required_fields if f not in data]
                if missing_fields:
                    self.log_test("Vulnerability Remediation", False, f"Missing remediation fields: {missing_fields}")
                    return False
                
                immediate_steps = data.get("immediate_steps", [])
                if not immediate_steps:
                    self.log_test("Vulnerability Remediation", False, "No immediate remediation steps provided")
                    return False
                
                self.log_test("Vulnerability Remediation", True, 
                            f"Retrieved remediation guidance for {data.get('vulnerability_name')} with {len(immediate_steps)} steps")
                return True
            elif response.status_code == 404:
                self.log_test("Vulnerability Remediation", False, "Vulnerability not found for remediation")
                return False
            else:
                self.log_test("Vulnerability Remediation", False, f"HTTP {response.status_code}: {response.text}")
                return False
                
        except Exception as e:
            self.log_test("Vulnerability Remediation", False, f"Error: {str(e)}")
            return False
    
    def test_vulnerability_rules_endpoint(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Check structure
                required_fields = ["total_rules", "rules_by_node_type", "rules"]
                missing_fields = [f for f in required_fields if f not in data]
                
                if missing_fields:
                    self.log_test("Vulnerability Rules", False, f"Missing fields: {missing_fields}")
                    return False
                ...
                # Verify rules for each node type
                expected_node_types = ["WebApp", "API", "Database"]
                for node_type in expected_node_types:
                    if node_type not in rules_by_type or rules_by_type[node_type] == 0:
                        self.log_test("Vulnerability Rules", False, f"No rules found for {node_type}")
                        return False
                
                # Verify OWASP Top 10 2023 coverage
                owasp_categories = [rule.get("owasp_category", "") for rule in rules]
                ...
                for owasp_cat in owasp_2023_categories:
                    if any(owasp_cat in rule_cat for rule_cat in owasp_categories):
                        covered_categories.append(owasp_cat)
                
                if len(covered_categories) < 5:  # At least 5 OWASP categories should be covered
                    self.log_test("Vulnerability Rules", False, f"Insufficient OWASP 2023 coverage: {covered_categories}")
                    return False
                
                self.log_test("Vulnerability Rules", True, 
                            f"Retrieved {total_rules} rules covering {len(covered_categories)} OWASP 2023 categories")
                return True
            else:
                self.log_test("Vulnerability Rules", False, f"HTTP {response.status_code}: {response.text}")
                return False
    ...
    def test_bulk_vulnerability_analysis(self):
        """Test POST /api/vulnerabilities/bulk-analyze"""
        try:
            # Create multiple nodes for bulk analysis
            nodes_data = [
                {
                    "node_id": str(uuid.uuid4()),
                    "node_type": "WebApp",
                    "questionnaire_responses": self.create_realistic_webapp_questionnaire_data(),
                    "position": {"x": 300, "y": 200}
                },
                {
                    "node_id": str(uuid.uuid4()),
                    "node_type": "API", 
                    "questionnaire_responses": self.create_realistic_api_questionnaire_data(),
                    "position": {"x": 500, "y": 300}
                },
                {
                    "node_id": str(uuid.uuid4()),
                    "node_type": "Database",
                    "questionnaire_responses": self.create_realistic_database_questionnaire_data(),
            ...
            if response.status_code == 200:
                ...
                missing_fields = [f for f in required_fields if f not in data]
                if missing_fields:
                    self.log_test("Bulk Vulnerability Analysis", False, f"Missing fields: {missing_fields}")
                    return False
                ...
                if analyzed_nodes != 3:
                    self.log_test("Bulk Vulnerability Analysis", False, f"Expected 3 nodes, got {analyzed_nodes}")
                    return False
                
                if successful_analyses != 3:
                    self.log_test("Bulk Vulnerability Analysis", False, f"Expected 3 successful analyses, got {successful_analyses}")
                    return False
                
                if total_vulnerabilities == 0:
                    self.log_test("Bulk Vulnerability Analysis", False, "No vulnerabilities found in bulk analysis")
                    return False
                
                # Verify each node has results
                for node_data in nodes_data:
                    node_id = node_data["node_id"]
                    if node_id not in results:
                        self.log_test("Bulk Vulnerability Analysis", False, f"Missing results for node {node_id}")
                        return False
                    
                    node_result = results[node_id]
                    if "error" in node_result:
                        self.log_test("Bulk Vulnerability Analysis", False, f"Error in node {node_id}: {node_result['error']}")
                        return False
    ...
    def test_vulnerability_deletion(self):
        """Test DELETE /api/vulnerabilities/{vulnerability_id}"""
        try:
            # First create a vulnerability analysis
            node_id = str(uuid.uuid4())
            questionnaire_data = self.create_realistic_webapp_questionnaire_data()
            ...
            if create_response.status_code != 200:
                self.log_test("Vulnerability Deletion", False, "Failed to create initial analysis")
                return False
            ...
            if not vulnerability_nodes:
                self.log_test("Vulnerability Deletion", False, "No vulnerabilities to test deletion")
                return False
            
            # Test deletion of first vulnerability
            vulnerability_id = vulnerability_nodes[0]["id"]
            initial_count = len(vulnerability_nodes)
            
            response = self.session.delete(f"{self.base_url}/vulnerabilities/{vulnerability_id}")
            
            if response.status_code == 200:
                data = response.json()
                
                if "message" not in data or "vulnerability_id" not in data:
                    self.log_test("Vulnerability Deletion", False, "Missing response fields")
                    return False
                
                # Verify vulnerability is removed from cache
                get_response = self.session.get(f"{self.base_url}/vulnerabilities/{node_id}")
                if get_response.status_code == 200:
                    updated_data = get_response.json()
                    updated_count = updated_data.get("total_vulnerabilities", 0)
                    
                    if updated_count != initial_count - 1:
                        self.log_test("Vulnerability Deletion", False, 
                                    f"Expected {initial_count - 1} vulnerabilities, got {updated_count}")
                        return False
                
                self.log_test("Vulnerability Deletion", True, 
                            f"Successfully deleted vulnerability {vulnerability_id}")
                return True
            elif response.status_code == 404:
                self.log_test("Vulnerability Deletion", False, "Vulnerability not found for deletion")
                return False
            else:
                self.log_test("Vulnerability Deletion", False, f"HTTP {response.status_code}: {response.text}")
                return False
                
        except Exception as e:
            self.log_test("Vulnerability Deletion", False, f"Error: {str(e)}")
            return False
    
    def test_owasp_top_10_coverage(self):
        """Test comprehensive OWASP Top 10 2023 vulnerability coverage"""
        try:
            # Test all three node types with insecure configurations
            test_cases = [
                ("WebApp", self.create_realistic_webapp_questionnaire_data()),
                ("API", self.create_realistic_api_questionnaire_data()),
                ("Database", self.create_realistic_database_questionnaire_data())
            ]
            ...
            covered_categories = []
            for expected_cat in expected_owasp_categories:
                if any(expected_cat in found_cat for found_cat in all_owasp_categories):
                    covered_categories.append(expected_cat)
            
            coverage_percentage = (len(covered_categories) / len(expected_owasp_categories)) * 100
            
            if coverage_percentage < 70:  # At least 70% coverage expected
                self.log_test("OWASP Top 10 Coverage", False, 
                            f"Insufficient OWASP coverage: {coverage_percentage:.1f}% ({len(covered_categories)}/10)")
                return False
            
            self.log_test("OWASP Top 10 Coverage", True, 
                        f"OWASP 2023 coverage: {coverage_percentage:.1f}% ({len(covered_categories)}/10 categories), "
                        f"{total_vulnerabilities} total vulnerabilities")
            return True
            
        except Exception as e:
            self.log_test("OWASP Top 10 Coverage", False, f"Error: {str(e)}")
            return False
    ...
    def test_questionnaire_completion_standalone(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if missing_fields:
                    self.log_test("Questionnaire Completion Standalone", False, 
                ...
                self.log_test("Questionnaire Completion Standalone", True, 
            ...
            self.log_test("Questionnaire Completion Standalone", False, f"❌ Error: {str(e)}")
            return False
    ...
    def test_questionnaire_get_with_security_branches(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Check for security_branches field specifically
                if "security_branches" not in data:
                    self.log_test("Questionnaire Get Security Branches", False, 
                ...
                self.log_test("Questionnaire Get Security Branches", True, 
            ...
            self.log_test("Questionnaire Get Security Branches", False, f"❌ Error: {str(e)}")
            return False
    ...
    def test_standalone_simulation(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if missing_fields:
                    self.log_test("Standalone Simulation", False, 
                ...
                self.log_test("Standalone Simulation", True, 
            ...
            self.log_test("Standalone Simulation", False, f"❌ Error: {str(e)}")
            return False
    ...
    def test_standalone_rule_evaluation(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if missing_fields:
                    self.log_test("Standalone Rule Evaluation", False, 
                                f"❌ Missing expected fields: {missing_fields}. Response fields: {list(data.keys())}")
                    return False
                ...
                self.log_test("Standalone Rule Evaluation", True, 
            ...
            self.log_test("Standalone Rule Evaluation", False, f"❌ Error: {str(e)}")
            return False
